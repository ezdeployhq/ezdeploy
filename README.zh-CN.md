...
<p align="center">
  <img src="./.github/assets/hero-zh.svg" width="100%" alt="EZdeploy — 对编码 Agent 说一句「部署到应用中心」，确认计划，获得你自己域名下经过健康检查的正式链接。">
</p>
...
- **一句话到生产环境。** 安装一次、保存一个 Key，之后的每次发布从一句话开始，而不是一套工具链。
- **发布前必须经你确认。** 每次部署都会预览运行时、资源绑定、访问范围与风险，执行内容与已确认的计划摘要绑定。
- **你的基础设施，你的凭证。** 一切都运行在你自己的 Cloudflare 账号里。云平台凭证以 Worker Secret 保存，绝不会进入应用仓库或 Agent 提示词。
- **为被 Agent 发现而建。** 规范工作流可通过 `agent.md`、可安装 Skill、`skill.md`、`agents.md`、`llms.txt`、`/.well-known/ezdeploy.json` 和 `openapi.json` 发现。
...
| 文档 | 内容 |
| --- | --- |
| [docs/cloudflare-setup.md](./docs/cloudflare-setup.md) | Cloudflare 令牌、Worker 安装、非交互参数 |
| [docs/architecture.md](./docs/architecture.md) | 控制面、Provider 与状态机设计 |
| [docs/ai-provider-management.md](./docs/ai-provider-management.md) | DeepSeek、OpenAI、Anthropic、Gemini、OpenRouter、Workers AI |
| [docs/account-management.md](./docs/account-management.md) | 单管理员账号、会话与凭证存储 |
| [docs/open-source-release.md](./docs/open-source-release.md) | 发布检查清单与仓库加固 |
| [CONTRIBUTING.md](./CONTRIBUTING.md) · [SECURITY.md](./SECURITY.md) · [SUPPORT.md](./SUPPORT.md) | 贡献、漏洞报告与获取帮助 |
...
具备终端能力的 Agent 会把带版本的独立客户端下载到操作系统临时目录；Remote MCP 和由运营者发布的 `@ezdeploy/agent` 包仍是可选的增强分发方式。用户不需要运行本地控制面守护进程，也不需要接触 Cloudflare Access Service Token。旧的 well-known 路径与 `ZAODEPLOY_*` 环境变量会继续作为稳定的兼容协议标识保留。
...
EZdeploy 正在为首个公开的 `0.1.0` 版本做准备，并采用 Apache-2.0 许可证。核心部署流程已经实现并通过测试；Cloudflare 安装仍需要使用部署者自己的基础设施，并完成安全审查。源码仓库位于 [ezdeployhq/ezdeploy](https://github.com/ezdeployhq/ezdeploy)。另请参阅 [CONTRIBUTING.md](./CONTRIBUTING.md)、[SECURITY.md](./SECURITY.md) 和[发布检查清单](./docs/open-source-release.md)。

...
<body><main class="shell"><h1>个人应用中心</h1><div class="sub">由 EZdeploy 发布并验证的个人应用</div><div id="login" class="bar"><input id="token" type="password" placeholder="本地模式个人 API Token"><button onclick="load()">进入</button></div><div id="message"></div><section id="apps" class="grid"></section></main>
<script>async function load(){const token=document.querySelector('#token').value;const headers=token?{authorization:'Bearer '+token}:{};const r=await fetch('/api/apps',{headers});if(!r.ok){document.querySelector('#message').innerHTML='<p class="error">无权访问，请填写个人 API Token。</p>';return}const rows=await r.json();document.querySelector('#login').style.display='none';document.querySelector('#message').innerHTML=rows.length?'':'<div class="empty">暂无已发布应用</div>';document.querySelector('#apps').innerHTML=rows.map(x=>'<article class="card"><span class="status">'+esc(x.deployment?.status||'未发布')+'</span><h2>'+esc(x.application.displayName)+'</h2><div class="meta">所有者：'+esc(x.application.ownerId)+'</div><div class="meta">环境：'+esc(x.environment.name)+' · 访问：'+esc(x.access?.mode||'未配置')+'</div><div class="meta">能力：'+(x.resources.map(r=>esc(r.kind)).join(' · ')||'无')+'</div><div class="versions"><strong>最近版本</strong><ul>'+x.recentDeployments.slice(0,5).map(d=>'<li><span>#'+d.sequence+'</span><span>'+esc(d.status)+'</span></li>').join('')+'</ul></div>'+(x.deployment?.url?'<a href="'+encodeURI(x.deployment.url)+'" target="_blank" rel="noreferrer">打开应用 →</a>':'')+'</article>').join('')}function esc(v){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}load()</script></body></html>`;

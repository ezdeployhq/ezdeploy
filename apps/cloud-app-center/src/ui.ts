...
<body>
  ...
  <script>
    ...
    async function loadProviders(){const r=await fetch('/api/ai/providers');const data=await r.json();if(!r.ok)throw Error(data.error?.message||'Provider 读取失败');state.providers=data;drawProviders()}
    ...
    async function testProvider(id){const r=await fetch('/api/ai/providers/'+id+'/test',{method:'POST'});const data=await r.json();await loadProviders();alert(data.message||data.error?.message||(r.ok?'连接成功':'连接失败'))}window.testProvider=testProvider;
    async function deleteProvider(id){if(!confirm('删除后，使用这个 Provider 的请求会立即切换到其他默认配置。继续吗？'))return;const r=await fetch('/api/ai/providers/'+id,{method:'DELETE'});if(!r.ok){const data=await r.json();return alert(data.error?.message||'删除失败')}await loadProviders()}window.deleteProvider=deleteProvider;
    document.querySelector('#providerForm').onsubmit=async event=>{event.preventDefault();const id=document.querySelector('#providerId').value;const body={providerType:providerType.value,name:document.querySelector('#providerName').value,baseUrl:document.querySelector('#providerBaseUrl').value,apiKey:document.querySelector('#providerApiKey').value,defaultModel:document.querySelector('#providerDefaultModel').value,models:document.querySelector('#providerModels').value.split(',').map(x=>x.trim()).filter(Boolean),enabled:document.querySelector('#providerEnabled').checked,isDefault:document.querySelector('#providerDefault').checked};const r=await fetch('/api/ai/providers'+(id?'/'+id:''),{method:id?'PUT':'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok){document.querySelector('#providerError').textContent=data.error?.message||'保存失败';return}document.querySelector('#providerDialog').close();await loadProviders()};
